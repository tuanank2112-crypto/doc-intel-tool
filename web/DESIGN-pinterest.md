---
version: alpha
name: VN-Gov-design-analysis
description: |
  A public-service system for a Vietnamese government app, organized around the National Flag Red (Cờ đỏ) CTA, the Star Gold (Sao vàng) accent, an information grid, and a clean cool-neutral chrome that keeps official content legible and calm. The home page is a service-discovery tool wearing the chrome of a state portal: large display headlines, friendly Pin Sans typography, 16px-radius buttons on a cool-neutral palette, and a sticky red primary CTA that anchors every viewport. Content cards (services, notices, documents) are the system's load-bearing element — they tile in a column-based grid where each tile is a 16px-radius card, separated by tight 8px gutters. The chrome is otherwise quiet: cool grays, true whites, a single saturated flag red, and a reserved star gold for badges/emphasis — no decorative gradients, no atmospheric backgrounds, no shadows beyond a soft modal scrim.

colors:
  primary: "#da251d"
  on-primary: "#ffffff"
  primary-pressed: "#b81e17"
  gold: "#ffcd00"
  gold-deep: "#e0b000"
  on-gold: "#3a2f00"
  ink: "#000000"
  ...
  stone: "#c1c4c8"
  hairline: "#d6d9dd"
  hairline-soft: "#e4e6e9"
  on-secondary: "#000000"
  secondary-bg: "#e4e6e9"
  secondary-pressed: "#c8ccd0"
  canvas: "#ffffff"
  surface-soft: "#f9fafb"
  surface-card: "#f3f4f6"
...
The Vietnamese government system is built around a single instructional principle: keep official content calm, legible, and trustworthy. The chrome is a quiet cool-neutral palette (`{colors.surface-soft}`, `{colors.surface-card}`, `{colors.canvas}`) carrying typography in the Pin Sans face, with Flag Red / Cờ đỏ (`{colors.primary}` — `#da251d`) reserved for primary CTAs, the active-tab indicator, and the sticky top-nav anchor, and Star Gold / Sao vàng (`{colors.gold}` — `#ffcd00`) reserved for official emphasis (emblem lockups, priority/honor badges). Every other surface stays neutral so the actual content — services, notices, forms, documents — reads clearly. The red-and-gold pairing echoes the national flag without turning the chrome into a flag: red carries at most one CTA per fold, gold is a badge accent only.
...
**Key Characteristics:**
- Flag-red CTA + star-gold emphasis: Flag Red (`{colors.primary}`) carries every primary action; Star Gold (`{colors.gold}`) marks official badges/seals only; everything else is monochrome
...
- Cool-neutral chrome (`{colors.surface-card}` — #f3f4f6) that recedes behind content without competing
- Sticky top nav with the always-red primary CTA anchored in the upper-right at every breakpoint
...
### Brand & Accent
- **Flag Red / Cờ đỏ** (`{colors.primary}` — `#da251d`): the national flag red — the system's primary saturated color. Primary CTAs, sticky top-nav anchor, active state in tab strips, and the brand/agency wordmark.
- **Flag Red Pressed** (`{colors.primary-pressed}` — `#b81e17`): pressed state for the primary button — a single notch deeper than flag red.
- **Star Gold / Sao vàng** (`{colors.gold}` — `#ffcd00`): the national flag's five-pointed-star gold. Reserved for emphasis badges, official seals/emblem lockups, honor/priority markers, and the star glyph in the wordmark. Never a body or CTA-fill color — it fails contrast on white for text.
- **Star Gold Deep** (`{colors.gold-deep}` — `#e0b000`): pressed/border pairing for gold badges and the readable edge of gold-on-white elements.
- **On Gold** (`{colors.on-gold}` — `#3a2f00`): dark brown-ink text placed on a gold fill (the only accessible text-on-gold pairing).
...
- **Surface Card** (`{colors.surface-card}` — `#f3f4f6`): cool-neutral card and tile background. Carries category tiles, search-bar default fill, button-secondary default, and content-card backgrounds.
- **Secondary BG** (`{colors.secondary-bg}` — `#e4e6e9`): the cool-gray used for `{component.button-secondary}` fill — a notch deeper than `{colors.surface-card}`.
- **Secondary Pressed** (`{colors.secondary-pressed}` — `#c8ccd0`): pressed state for the secondary button.
- **Surface Dark** (`{colors.surface-dark}` — `#7a1410`): deep flag-red maroon used for the official dark CTA strip / masthead band (evokes the flag ground rather than a neutral black).
- **Hairline** (`{colors.hairline}` — `#d6d9dd`): 1px row dividers, footer column rules.
- **Hairline Soft** (`{colors.hairline-soft}` — `#e4e6e9`): lighter inline divider; doubles as the secondary-button background.

### Text
- **Ink** (`{colors.ink}` — `#000000`): primary headlines, button text, primary nav links.
- **Ink Soft** (`{colors.ink-soft}` — `#1a1f2b`): inline-link color in body prose — a near-black with a faint cool cast.
- **Body** (`{colors.body}` — `#2e3138`): default paragraph text on `{colors.canvas}`.
- **Charcoal** (`{colors.charcoal}` — `#22252b`): subtly softer body where pure ink is too heavy.
- **Mute** (`{colors.mute}` — `#5b5f66`): metadata text, footer links, secondary captions.
- **Ash** (`{colors.ash}` — `#8c9096`): disabled button text, placeholder text in inputs.
- **Stone** (`{colors.stone}` — `#c1c4c8`): least-emphasis utility text, disabled-state borders.
- **On Dark** (`{colors.on-dark}` — `#ffffff`): primary text on `{colors.surface-dark}`.

### Semantic
- **Error** (`{colors.error}` — `#9e0a0a`): validation messages, destructive confirmation copy. Kept distinct from `{colors.primary}` flag red so warnings never read as brand.
- **Error Deep** (`{colors.error-deep}` — `#b81e17`): deepened error background where the regular error tone needs more contrast. Matches the primary-pressed value but in error context represents semantic destructiveness.
- **Info** (`{colors.info}` — `#0b63a5`): official notice / informational banners and status pills (announcements, procedural guidance).
- **Success Deep** (`{colors.success-deep}` — `#0f5132`): success messaging (form submitted, request approved).
- **Success Pale** (`{colors.success-pale}` — `#c8f0d5`): pale success-pill background.
...
### Institutional Accents (used sparingly for badges, seals, and informational chrome)
- **Star Gold** (`{colors.gold}` — `#ffcd00`): official emphasis — priority/honor badges, the emblem-star lockup, decorative seal edges. Pair with `{colors.on-gold}` text; never use gold for body text or CTA fills.
- **Accent Blue** (`{colors.accent-blue}` — `#1d4ed8`): informational badges, links to procedures/laws, non-brand secondary actions in dense forms.
- **Accent Blue Deep** (`{colors.accent-blue-deep}` — `#1e3a8a`): pressed pairing for blue badges and administrative-section iconography.
...
- Background `{colors.primary}` (Flag Red), text `{colors.on-primary}`, type `{typography.button-md}`, padding `6px 14px`, height ~40px, rounded `{rounded.md}` (16px).
- Used for "Sign up", "Join Pinterest for free", "Get started" — every primary action across every surface in the system.
- Pressed state lives in `button-primary-pressed` — background drops to `{colors.primary-pressed}` (`#b81e17`).
...
- Layout (desktop home): agency wordmark + emblem (star-gold accent) at left + primary section link, centered `{component.search-bar}`, right cluster ("Dịch vụ / Thủ tục / Tin tức / Đăng nhập" + the always-red `{component.button-primary}` primary CTA).
- Layout (results): agency emblem/logo at left, centered search bar with the active query, right cluster ("Đăng nhập" + red primary button).
...
- Bottom row: agency wordmark + emblem + "© 2026 Cơ quan chủ quản" in `{typography.caption-sm}` `{colors.mute}`.

### Inline

**`link-inline`** — body-prose anchor link
- `{colors.ink-soft}` text with no underline by default — a near-black cool tint used inline to differentiate links from body without color contrast. For links to laws/procedures, `{colors.accent-blue}` may be used instead.

## Do's and Don'ts

### Do
- Reserve `{colors.primary}` (Flag Red) for primary CTAs, the active-tab indicator, and the brand/agency wordmark only. It is never decorative.
- Reserve `{colors.gold}` (Star Gold) for official emphasis — emblem lockups, priority/honor badges, seal edges — always paired with `{colors.on-gold}` text. Never use gold as a CTA fill or body-text color.
...
- Don't replace `{colors.primary}` with another red. The flag red is precise — `#da251d`. Don't tint it toward the Pinterest/crimson `#e60023`.
- Don't use `{colors.gold}` for text on white or as a large fill — it fails contrast. Gold appears as a badge/seal fill with `{colors.on-gold}` text, or as a small glyph/edge only.
