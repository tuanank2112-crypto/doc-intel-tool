...
(function () {
  ...
  /** Chuỗi lỗi từ API — tránh "Không trả lời được: [object Object]" */
  function errText(payload, fallback) {
    if (payload == null || payload === "") return fallback || "lỗi không rõ";
    ...
    if (typeof payload === "object") {
      ...
      if (payload.detail && typeof payload.detail === "object") {
        return errText(payload.detail, fallback);
      }
      try {
        return JSON.stringify(payload);
      } catch (e) {
        return fallback || "lỗi không rõ";
      }
    }
  ...
  function renderStructuredBody(text, anchorId, ctx) {
    ...
    function _mhQuoc(ln) {
      return /CỘNG\s*HÒA\s*XÃ\s*HỘI\s*CHỦ\s*NGHĨA\s*VIỆT\s*NAM/i.test(ln);
    }
    function _mhTieu(ln) {
  ...
  function hookWhenReady() {
    ...
    window.addQ = function (q) {
      ...
      fetch(API + "/v1/ask", {
        ...
        .then(function (x) {
          if (!x.ok) {
            throw new Error(errText(x.j, "ask failed"));
          }
          var j = x.j || {};
          if (j.error) {
            throw new Error(errText(j, j.error));
          }
          var cites = (j.citations || [])
            ...
            .join("");
          var modeLabel =
            j.answer_mode === "heuristic_search" ||
            j.answer_mode === "heuristic_fallback" ||
            j.llm_used === false
              ? "Tìm kiếm thô (chưa LLM / fallback)"
        ...
        .catch(function (e) {
          ...
          done("Không trả lời được: " + esc(msg));
        });
    };
  ...
  /* health badge on login removed (không hiện model/gemini trên popup) */
